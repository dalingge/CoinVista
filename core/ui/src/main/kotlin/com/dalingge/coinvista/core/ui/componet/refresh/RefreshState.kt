package com.dalingge.coinvista.core.ui.componet.refresh
...
fun rememberRefreshState(
    ...
    shouldEnableRefresh: () -> Boolean = { true },
): RefreshState {
    return remember { RefreshState(coroutineScope,shouldEnableRefresh) }
}
...
class RefreshState(
    ...
    private val shouldEnableRefresh: () -> Boolean,
) {
    /**
     * 刷新头部的触发高度阈值
     */
    ...
    internal val connection = object : NestedScrollConnection {
        ...
        override fun onPostScroll(
            ...
            // 【核心修复】增加方向判断
            if (!shouldEnableRefresh() || available.x.absoluteValue  > available.y.absoluteValue ) {
                return Offset.Zero
            }
            return when {
                enableRefresh && available.y > 0 -> {
